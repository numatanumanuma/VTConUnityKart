...
public class ROSTransformTreePublisher : MonoBehaviour
{
    ...
    public static uint seq = 0; // ROS1用
    ...
    void PublishMessage()
    {
        ...
        if (m_GlobalFrameIds.Count > 0)
        {
            var tfRootToGlobal = new TransformStampedMsg(
                // new HeaderMsg(new TimeStamp(Clock.time), m_GlobalFrameIds.Last()),   // ROS2
                new HeaderMsg(seq, new TimeStamp(Clock.time), m_GlobalFrameIds.Last()), // ROS1
        ...
        for (var i = 1; i < m_GlobalFrameIds.Count; ++i)
        {
            var tfGlobalToGlobal = new TransformStampedMsg(
                // new HeaderMsg(new TimeStamp(Clock.time), m_GlobalFrameIds[i - 1]), // ROS2
                new HeaderMsg(seq, new TimeStamp(Clock.time), m_GlobalFrameIds[i - 1]),  // ROS1
                m_GlobalFrameIds[i],
